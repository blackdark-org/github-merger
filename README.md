...
A pull request merges only when it has every required label, none of the blocked labels, is not a draft, and its head branch is in the same repository. `mergeable` must be true. Every check suite that is in progress, or queued with at least one check run, must be completed. A queued suite with no runs does not block. The newest suite must be at least `settle` old, and the highest-id run of each check name must be `success` or `skipped`. A pull request with no check runs is skipped unless `merge_without_checks_after` is set and the newest suite is at least that old, for example when path filters start no workflow. A `squash` label selects a squash merge. Otherwise the merge method is `default_merge_method` (`merge` by default).
...
Open pull requests without the required labels, with a blocked label, or in draft are skipped from the list response, so they cost no further API calls. Unlabeled skips log at debug. Failed reads are retried twice (after 1s and 2s) on 5xx, 429, or network errors. A merge call is never retried.
...
The image is `ghcr.io/blackdark-org/github-merger`. Use `0.1.3` for this release, or `0.1` to follow the 0.1 line. The process runs as UID 65532 and does not write to disk, so the mounted PEM and config must be readable by that user.
...
docker run -d --name github-merger --restart unless-stopped \
  ...
  ghcr.io/blackdark-org/github-merger:0.1.3
```

```yaml
services:
  github-merger:
    image: ghcr.io/blackdark-org/github-merger:0.1.3
